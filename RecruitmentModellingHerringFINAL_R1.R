...
springrecN<-subset(springrecN, year>1981) # Omit years 1978-1981 to have consistent time series

########Load age structure data, proportion of age group compared to total age 4+
Sagestruct<-read.csv("SpringAgeProp.csv")
Sagestruct<-subset(Sagestruct, year>1981)
...
Sage8to11T<-subset(Sage8to11T, year>1981) # Omit years 1978-1981 to have consistent time series
excel(Sage8to11T)
...
FallrecN<-subset(FallrecN, year>1981) #Omit 1978-1981 to maintain consistent timeseries with physical variables
#Load age structure data, proportion of age group compared to total age 4+
...
Fage8to11T<-subset(Fage8to11T, year>1981)
excel(Fage8to11T)
mean(Fage8to11T$Page8to11)
Fage8to11T[which.max(Fage8to11T$Page8to11),]
Fage8to11T[which.min(Fage8to11T$Page8to11),]
Fage8to11T<-subset(Fage8to11T, year>1981) #Omit 1978-1981 to maintain consistent timeseries with physical variables
...
VariablesT<-EA.query.f(years=1982:2019, variables=c( "decrease.10", "decrease.12", "first ice", "ice.duration", "ice.max", "last.ice", "sst", "sst.anomaly",
                                                   "sst.month10", "sst.month11", "sst.month5", "sst.month6", "sst.month7", "sst.month8", "sst.month9", "start.10",
                                                   "start.12","t.shallow"), EARs=c(3,5,6))
...
plot(NULL, xaxt = "n", yaxt = "n", xaxs = "i", yaxs = "i", xlab = "", ylab = "", xlim = c(1980, max(years)+1),
     ylim = c(0, 2000))
axis(1, at = seq(1980, max(years)+1, by = 1), tck = -0.008, label = F)
axis(1, at = seq(1980, 2020, by = 5), label = F)
